---
title: "Integration test for MRSea 2d salsa with other variables, no 1d smooth, no panels, no interactions"
...
## Introduction
This is the fifth in a series of tests that can be used to check changes to the MRSea package do not break previously working elements. They replicate the case study in the vignette but broken down into chunks. This fifth one checks the 2D salsa functions with other variables, no 1d smooths, no panels and no interactions.
...
pred_resp <- ggplot() + stat_summary_2d(data=predictData, aes(predictData$x.pos, predictData$y.pos, z=preds_out), fun=max, binwidth=c(0.5,0.5)) + theme_bw() + scale_fill_distiller(palette = "Spectral", name="Animal Counts", limit=c(0, 6.1)) + coord_fixed(ratio = 1) + xlab("X coordinate") + ylab("Y coordinate")

predict_read_in <- read.csv("5.predict.data.csv")

resp_read <- ggplot() + stat_summary_2d(data=predict_read_in, aes(x.pos, y.pos, z=ResponsePredictions), fun=max, binwidth=c(0.5,0.5)) + theme_bw() + scale_fill_distiller(palette = "Spectral", name="Animal Counts", limit=c(0, 6.1)) + coord_fixed(ratio = 1) + xlab("X coordinate") + ylab("Y coordinate")
...
pred_link <- ggplot() + stat_summary_2d(data=predictData, aes(predictData$x.pos, predictData$y.pos, z=preds_out_link), fun=max, binwidth=c(0.5,0.5)) + theme_bw() + scale_fill_distiller(palette = "Spectral", name="Nhat on link scale", limit=c(-7.1, 3.2)) + coord_fixed(ratio = 1) + xlab("X coordinate") + ylab("Y coordinate")

link_read <- ggplot() + stat_summary_2d(data=predict_read_in, aes(x.pos, y.pos, z=LinkPredictions), fun=max, binwidth=c(0.5,0.5)) + theme_bw() + scale_fill_distiller(palette = "Spectral", name="Nhat on link scale", limit=c(-7.1, 3.2)) + coord_fixed(ratio = 1) + xlab("X coordinate") + ylab("Y coordinate")
...
cat("Sum of all predictions on response scale:", sum(preds_out), "this should be 17208.22. \n\n")

cat("Sum of all predictions on link scale:", sum(preds_out_link), "this should be -33993.69. \n\n")
...
```{r, echo=FALSE}
sum_results_read_in <- read.csv("5.sum.results.csv")
...
cat("Sum of all estimates:", sum(sum_results$coefficients[,1]), "this should be 19.92147. \n\n")

cat("Sum of all robust SE:", sum(sum_results$coefficients[,3]), "this should be 3.767193. \n\n")

cat("Dispersion:", sum_results$dispersion, "this should be 8.893862. \n\n")
