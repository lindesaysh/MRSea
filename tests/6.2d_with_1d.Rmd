...
pred_resp <- ggplot() + stat_summary_2d(data=predictData, aes(predictData$x.pos, predictData$y.pos, z=preds_out), fun=max, binwidth=c(0.5,0.5)) + theme_bw() + scale_fill_distiller(palette = "Spectral", name="Animal Counts", limit=c(0, 16.9)) + coord_fixed(ratio = 1) + xlab("X coordinate") + ylab("Y coordinate")

predict_read_in <- read.csv("6.predict.data.csv")

resp_read <- ggplot() + stat_summary_2d(data=predict_read_in, aes(x.pos, y.pos, z=ResponsePredictions), fun=max, binwidth=c(0.5,0.5)) + theme_bw() + scale_fill_distiller(palette = "Spectral", name="Animal Counts", limit=c(0, 16.9)) + coord_fixed(ratio = 1) + xlab("X coordinate") + ylab("Y coordinate")
...
cat("Sum of all estimates:", sum(sum_results$coefficients[,1]), "this should be 20.37099. \n\n")

cat("Sum of all robust SE:", sum(sum_results$coefficients[,3]), "this should be 16.54627. \n\n")
