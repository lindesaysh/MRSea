...
pred_resp <- ggplot() + stat_summary_2d(data=predictData, aes(predictData$x.pos, predictData$y.pos, z=preds_out), fun=max, binwidth=c(0.5,0.5)) + theme_bw() + scale_fill_distiller(palette = "Spectral", name="Animal Counts", limit=c(0, 3.4)) + coord_fixed(ratio = 1) + xlab("X coordinate") + ylab("Y coordinate") + ggtitle("New predictions")
...
predict_read_in <- read.csv("4.predict.data.15.csv")

resp_read <- ggplot() + stat_summary_2d(data=predict_read_in, aes(x.pos, y.pos, z=ResponsePredictions), fun=max, binwidth=c(0.5,0.5)) + theme_bw() + scale_fill_distiller(palette = "Spectral", name="Animal Counts", limit=c(0, 3.4)) + coord_fixed(ratio = 1) + xlab("X coordinate") + ylab("Y coordinate") + ggtitle("Saved predictions")

knot_loc_read <- read.csv("4.knot.loc.15.csv")
...
link_read <- ggplot() + stat_summary_2d(data=predict_read_in, aes(x.pos, y.pos, z=LinkPredictions), fun=max, binwidth=c(0.5,0.5)) + theme_bw() + scale_fill_distiller(palette = "Spectral", name="Nhat on link scale", limit=c(-18.6, 2.7)) + coord_fixed(ratio = 1) + xlab("X coordinate") + ylab("Y coordinate") + ggtitle("Saved predictions")
...
cat("Sum of all predictions on response scale:", sum(preds_out), "this should be 2166.568. \n\n")

cat("Sum of all predictions on link scale:", sum(preds_out_link), "this should be -8723.428. \n\n")

cat("Time for training:", round(total_time, 1), "Original time: 12.6. \n\n")
...
cat("Number of model fits:", n_fits, "Original model fits: 794")
...
```{r, echo=FALSE}
sum_results_read_in <- read.csv("4.sum.results.15.csv")
...
cat("Sum of all estimates:", sum(sum_results$coefficients[,1]), "this should be -20.7902. \n\n")

cat("Sum of all robust SE:", sum(sum_results$coefficients[,3]), "this should be 217.6414. \n\n")

cat("Standard errors are equal to robust standard errors:", sum(sum_results$coefficients[,2])==sum(sum_results$coefficients[,3]), "\n\n")

cat("Dispersion:", sum_results$dispersion, "this should be 13.85382. \n\n")
...
cat("Mean square error:", mean_sum_sq, "this should be 0.6593585. \n\n")

mean_sum_sq_data <- sum((data$response - predict.gamMRSea(newdata=data, object=salsa2dOutput$bestModel))^2)/nrow(data)

cat("residual sum square error to data:", mean_sum_sq_data, "this should be 6.014707. \n\n")
...
cat("Mean square error to truth seen:", mean_sum_sq_seen, "this should be 0.7262826. \n\n")

mean_sum_sq_unseen <- sum((predictData$truth.re[!rows_seen] - preds_out[!rows_seen])^2)/nrow(predictData[!rows_seen,])

cat("Mean square error to truth unseen:", mean_sum_sq_unseen, "this should be 0.6378279. \n\n")
