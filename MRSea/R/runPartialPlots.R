...
#' runPartialPlots(model, ns.data.re, factorlist=c('floodebb', 'impact'), 
#'                 varlist=c('observationhour'), type='link')
#' runPartialPlots(model, ns.data.re, factorlist=c('floodebb', 'impact'), 
#'                 varlist=c('observationhour'), partial.resid=TRUE)
#'
#' @export
#' 

runPartialPlots<-function(model, data, factorlist=NULL, varlist=NULL, showKnots=FALSE, type='response', partial.resid=FALSE, save=FALSE, savedata=F){
  ...
  if(is.null(factorlist)==F){
      for(i in 1:length(factorlist)){
        ...
        if(is.null(dim(rpreds))){
          ...
          if(partial.resid==T){
            ...
            par.res <- par.res[, c(grep(factorlist[i], colnames(par.res)))]
            if(type=='response'){
              par.res<-model$family$linkinv(par.res)
              y.lab = "Partial Fit (response)"
            }else{y.lab="Partial Fit (link)"}
            plot(xvals, par.res, xlab=factorlist[i], ylab=y.lab, ylim=range(c(0, cis, par.res)), xaxt="n", cex.lab=1.3, cex.axis=1.3, col='grey', cex=0.5)
          ...
          plot(newX, partialfit, pch=20,
               xlab=factorlist[i], ylab=y.lab, lwd=2, xaxt="n", ylim=c(range(0,cis)), cex.lab=1.3, cex.axis=1.3)
          }
          ...
          plot(1:length(newX), partialfit, pch=20,
               xlab=factorlist[i], ylab=y.lab, lwd=2, xaxt="n", ylim=range(c(0,cis)), cex.lab=1.3, cex.axis=1.3)
          }
    ...
    if(is.null(varlist)==F){
      ...
      for(i in 1:n){
        ...
        if(partial.resid==T){
          par.res <- residuals(model, type='partial')
          par.res <- par.res[, c(grep(varlist[i], colnames(par.res)))]
          if(type=='response'){
            par.res<-model$family$linkinv(par.res)
            y.lab = "Partial Fit (response)"
          }else{y.lab="Partial Fit (link)"}
          plot(xvals, par.res, xlab=varlist[i], ylab=y.lab, ylim=range(c(cis, par.res)), cex.lab=1.3, cex.axis=1.3, col='grey', cex=0.5)
          ...
          plot(newX, partialfit, type="l",
               xlab=varlist[i], ylab=y.lab, lwd=2, ylim=range(cis), cex.lab=1.3, cex.axis=1.3)
        }
        rug(xvals)
