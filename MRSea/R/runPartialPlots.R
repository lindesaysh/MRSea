...
runPartialPlots<-function(model, data, factorlist=NULL, varlist=NULL, showKnots=FALSE, save=FALSE){
  
  print("Making partial plots")
  
  if(save==T){png("PartialFits%i.png", height=600, width=650)}else{devAskNewPage(ask=TRUE)}
  par(mfrow=c(1,1))
  ...
  if(is.null(factorlist)==F){
    for(i in 1:length(factorlist)){
      ...
      newX<- sort(unique(as.numeric(xvals)))
