...
if (length(knotSites) > maxSites) knotSites = quantile(knotSites,probs=seq(0,1,length=maxSites),na.rm=TRUE,names=FALSE)
...
#print(knotSites)
    ...
    ###########################initialisation######################################
    output <- initialise.measures(startKnots, explanatory, response, degree, wts, initialise, initialKnots,baseModel,knotSites, bd, spl, fitnessMeasure, interactionTerm)
    point <- output$point
    ...
    ###################################exchange step#############################
      output <- exchange.step(degree, gap, response,explanatory,maxIterations,fitnessMeasure,point,knotPoint,position,aR,measures,
                                 out.lm,improveEx,maxKnots,winHalfWidth,wts, baseModel,knotSites,models, bd, spl, interactionTerm )
      point <- output$point
      knotPoint <- output$knotPoint
      position <- output$position
      aR <- output$aR
    ...
    #####################################improve step############################
      output <- improve.step(degree, gap, length(aR), response,explanatory,maxIterations,fitnessMeasure,point,knotPoint,position,
                                aR,measures,out.lm,improveNudge, wts, baseModel,knotSites,models, bd, spl, interactionTerm)
      point <- output$point
      knotPoint <- output$knotPoint
      position <- output$position
      aR <- output$aR
    ...
    ###################################drop step#################################
      if (length(aR) > minKnots) {
         output <- drop.step(degree, response,explanatory,maxIterations,fitnessMeasure,point,knotPoint,position,aR,measures,
                                  out.lm,improveDrop,minKnots, wts, baseModel,models, bd, spl, interactionTerm)
         point <- output$point
         knotPoint <- output$knotPoint
         position <- output$position
         aR <- output$aR
...
"initialise.measures" <- function(num,explanatory, response, degree, wts,  initialise, initialKnots,baseModel,knotSites, bd, spl, fitnessMeasure, interactionTerm){
  ...
  attributes(baseModel$formula)$.Environment<-environment()
    ...
    output <- fit.model(explanatory,degree,aR,baseModel,models, bd, spl, fitnessMeasure, interactionTerm)
    out.lm<-output$currentModel
    models<-output$models
    model<<-out.lm
    measures<- get.measure(fitnessMeasure,measures=NULL,out.lm)$fitStat
...
"exchange.step" <- function(degree, gap, response,explanatory,maxIterations,fitnessMeasure,point,knotPoint,position,aR,
                               measures,out.lm,improveEx,maxKnots,winHalfWidth,wts, baseModel,knotSites,models, bd, spl, interactionTerm){
      ...
      if (length(index)>0) {
        if (index > 0) {
           output <- move.knot(degree, index,fitnessMeasure,measures,aR,point,response,explanatory,out.lm,improve,improveEx,
                                  maxKnots, wts,  baseModel,knotSites,models, bd, spl, interactionTerm)
...
"locate.max.res" <- function(point,position,gap,response,explanatory, bd,winHalfWidth,out.lm,knotPoint,aR,wts,knotSites, spl){
   ...
   if (max(abs(tempRes))==0) {
      ...
      print(maxInd)
      siteIndex = which(abs(knotSites-explanatory[maxInd])==min(abs(knotSites[position>0]-explanatory[maxInd])))
...
"move.knot" <- function(degree, index,fitnessMeasure,measures,aR,point,response,explanatory,out.lm,improve,improveEx,maxKnots,
                            wts, baseModel,knotSites,models, bd, spl, interactionTerm){
  attributes(baseModel$formula)$.Environment<-environment()
print("Moving knot...")
    chck<-c()
    ...
    for (i in 1:length(aR)) {
       ...
       output <- fit.model(explanatory,degree,tempR,baseModel,models, bd, spl, fitnessMeasure, interactionTerm)
       if (output$currentModel$converge) {
          tempOut.lm<-output$currentModel
          models<-output$models
          output<-get.measure(fitnessMeasure,measures,tempOut.lm)
...
"improve.step" <- function(degree, gap, num,response,explanatory,maxIterations,fitnessMeasure,point,knotPoint,position,aR,
                                measures,out.lm,improveNudge,wts, baseModel,knotSites,models, bd, spl, interactionTerm){
  attributes(baseModel$formula)$.Environment<-environment()
print("Improving...")
   improve <- 1
   fuse <- 0
   while ( (improve) & (fuse < maxIterations) ) {
     ...
     for (i in 1:length(aR)) {
       ...
       output <- local.shift.up(degree, out.lm,i,knotPoint,gap,position,fitnessMeasure,measures,aR,point,response,explanatory,
                                    improve,improveNudge,wts, baseModel,knotSites,models, bd, spl, interactionTerm)
       point <- output$point
       position <- output$position
       knotPoint <- output$knotPoint
       aR <- output$aR
       ...
       output <- local.shift.down(degree, out.lm,i,knotPoint,gap,position,fitnessMeasure,measures,aR,point,response,explanatory,
                                       improve,improveNudge,wts,baseModel,knotSites,models, bd, spl, interactionTerm)
       point <- output$point
       position <- output$position
       knotPoint <- output$knotPoint
...
"drop.step" <- function(degree, response,explanatory,maxIterations,fitnessMeasure,point,knotPoint,position,aR,measures,out.lm,
                          improveDrop,minKnots,wts, baseModel,models, bd, spl, interactionTerm) {
...
getCV_type2<- function(folds, baseModel){
 ...
 "fit.model" <- function(explanatory,degree,aR,baseModel,models, bd, spl, fitnessMeasure, interactionTerm) {
  ...
  if(spl == 'bs'){
    ...
    print("model fitted...")
    if (out.lm$converge) {
      tempFit <- get.measure(fitnessMeasure, NA, out.lm)$fitStat
