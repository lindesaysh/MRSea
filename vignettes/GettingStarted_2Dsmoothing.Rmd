...
Depending on the size of your dataset this could take while and be slightly different every time you run it so I suggest using a seed and perhaps saving the knotgrid as a file.
...
1. Distance sampling, simple interactions, `gamMRSea` model diagnostics, inference via bootstrapping and using robust standard errors to deal with residual correlation see the [Full Case Study](https://lindesaysh.github.io/MRSea/article/RenewablesCaseStudy_MRSea_v1.3).
2. More complex [interactions](https://lindesaysh.github.io/MRSea/article/Interactions_salsa2d_MRSea_v1.3).
3. Using non Euclidean distances to measure the similarity between points. For example, using geodesic distances to represent "as the fish swims" rather than "as the crow flies".  This takes into account distances around exclusion areas such as coastlines or lakes. See [here]((https://lindesaysh.github.io/MRSea/article/Geodesic_2dsmoothing_MRSea_v1.3))
