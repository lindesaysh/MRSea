...
Package versions used here are - `statmod` Version `r sessionInfo()$otherPkgs$statmod$Version` and `tweedie` Version `r sessionInfo()$otherPkgs$tweedie$Version`
...
Note that the two model outputs are identical. In reality though we need to find out what value of $\xi$ is best for our data.  For this we can use the `tweedie_profile` function from the `tweedie` library. 

Note: this function can take a long time to run. 

```{r}
profout <- tweedie_profile(birds ~ 1, 
                           data=ns.data.re,
                           xi.vec = seq(1.01, 1.99, by=0.1), 
                           do.plot=TRUE,
                           do.smooth = FALSE,
                           do.ci = FALSE)
```
...
```{r}
profout2 <- tweedie_profile(birds ~ MonthOfYear + x.pos + y.pos + Year, 
                           data=ns.data.re,
...
Remember to specify a Tweedie specific fitness measure of either `tweedie_AIC` or `tweedie_BIC`.

```{r}
# set some input information for SALSA
salsa1dlist<-list(fitnessMeasure = 'tweedie_BIC', 
                  minKnots_1d = c(1), 
                  maxKnots_1d = c(3), 
                  startKnots_1d = c(1), 
                  degree = c(2),
...
```{r}
profout <- tweedie_profile(response ~ x.pos + y.pos + depth, 
                           data=baselinedata,
                           xi.vec = seq(1.01, 1.99, by=0.05), do.plot=TRUE)
```
