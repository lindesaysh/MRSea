...
The variance of the Tweedie distribution is parameterised by the mean $\mu$ and the dispersion parameter $\phi$:
...
cv.gamMRSea(ns.data.re, salsa1dOutput$bestModel, K = 10)$delta[2]
```
...
```
```{r}
cutpts<-unique(quantile(fitted(salsa1dOutput$bestModel), prob=seq(0,1, by=0.05)))
mycuts<-cut(fitted(salsa1dOutput$bestModel), breaks= cutpts)
meanfits<-tapply(fitted(salsa1dOutput$bestModel), mycuts, mean)
varresid<-tapply(residuals(salsa1dOutput$bestModel, type='response'), mycuts, var)
  
plot(meanfits, varresid, xlab='Fitted Values', ylab='V(residuals)')
abline(0,1)
lines(meanfits, meanfits*summary(salsa1dOutput$bestModel)$dispersion, col='red')
lines(meanfits, (meanfits^profout2$xi.max)*summary(salsa1dOutput$bestModel)$dispersion, col='blue')
```
...
```{r}
profout <- tweedie.profile(response ~ year + impact + x.pos + y.pos + depth, 
                           data=baselinedata,
                           xi.vec = seq(1.01, 1.99, by=0.05), do.plot=TRUE)
```
...
```{r}
cutpts<-unique(quantile(fitted(salsa2dOutput$bestModel), prob=seq(0,1, by=0.05)))
mycuts<-cut(fitted(salsa2dOutput$bestModel), breaks= cutpts)
meanfits<-tapply(fitted(salsa2dOutput$bestModel), mycuts, mean)
varresid<-tapply(residuals(salsa2dOutput$bestModel, type='response'), mycuts, var)
  
plot(meanfits, varresid, xlab='Fitted Values', ylab='V(residuals)')
abline(0,1)
lines(meanfits, meanfits*summary(salsa2dOutput$bestModel)$dispersion, col='red')
lines(meanfits, (meanfits^profout2$xi.max)*summary(salsa2dOutput$bestModel)$dispersion, col='blue')
```
