...
For a reliable model, we need to ensure we don't include variables which are too similar i.e. **collinear**.
Let's plot the relationship between all pair-wise covariates for the windfarm dataset.
...
```{r, eval=ifelse(opts_knit$get("rmarkdown.pandoc.to")=="html", TRUE, FALSE), echo=FALSE, fig.cap="Linear fit on fictitious data with independent covariates."}
rgl::plot3d(x1, x2, y)
rgl::planes3d(a=coefs["x1"], b=coefs["x2"], c=-1, 
         d=coefs["(Intercept)"], col='#d9d9d9', alpha=0.3)
rgl::view3d(0, -45)
...
```{r, eval=ifelse(opts_knit$get("rmarkdown.pandoc.to")=="html", TRUE, FALSE), echo=FALSE, fig.cap="Linear fit on fictitious data with covariates of varying levels of correlation.", fig.width=8, fig.height=8}
# Display result of each model 
# and plot hyperplane and data points
rgl::open3d()
rgl::close3d()
...
As the correlation coefficient between $x_1$ and $x_2$ increases, so do the standard errors of the regression coefficients.
Up until $\rho=1$, when $x_1$ and $x_2$ are colinear i.e. their data points lie on a single line in the $x_1/x_2$ space, and the model fails to fit due to singularities. 
...
Collinearity can be diagnosed by plotting the covariates against each other, but it can be automated 
by employing numerical measures (e.g. VIFs). This is particularly useful when dealing with a large number of covariates.
...
Including `DepthSonar` and `DepthLiDAR` results in a GVIF of `r round(car::vif(glmFitColinear)["DepthSonar", "GVIF^(1/(2*Df))"], digits=2)`, confirming this collinearity problem and the associated issues with model stability.


### Dealing with collinearity

What do you do when you have collinearity present?  The two commonest options are to centre covariates or to omit either of the troublesome covariates from the model completely. In many cases, centering the covariate doesn't help and dealing with collinearity necessarily involves some model selection^[One can also use multivariate statistical techniques, such as principal component analysis (PCA), to project the covariates into a new feature space where they are uncorrelated.].  You can fit models with either one of the variables and use some kind of criterion (e.g. AIC or cross-validation) to choose the better of the two models.  
...
We can examine this linearity (on the link scale) assumption by plotting the Pearson residuals against
each of our continuous covariates. The Pearson residuals are the "raw" residuals (response residuals) divided by the square root of the variance function $v(\cdot)$. So for the Poisson model $\hat{\epsilon_i}^{\text{pearson}}=\frac{y_i-\hat{y_i}}{\sqrt{\hat{y_i}}}$ 

Recall, if the relationship is not well described by the model, the Pearson residuals will exhibit a systematic pattern.
...
There is an obvious systematic pattern that tells us that our linear predictor for the mean is not doing a good job at describing the observed data.
Let's include $x^2$ as another covariate in the model and see what happens.

```{r}
df$xsq <- x^2 # "new" covariate x-squared
mdl <- glm(y ~ x + xsq, data=df)
summary(mdl)
```
...
Now there is no structure in the Pearson residuals. They are all scattered evenly around zero, suggesting that our model's
assumptions are met and we have a good fit.
...
Moreover, these residual plots can be augmented with a data-driven curve. The details of these smooth curves will be covered in later, but can be thought of as a kind of running average, where a window is moved across the covariate range the average of the values inside each window is found. These average values are then joined across windows (in clever ways) to make a fitted curve. These fitted curves are known as **non-parametric** models because they rely exclusively on the data rather than some pre-specified parametric form for the systematic model component.

The handy [`residualPlots`](https://www.rdocumentation.org/packages/car/versions/3.0-6/topics/residualPlots) function in the `car` package lets us do all of the above; plotting the Pearson residuals against all other covariates, compute a curvature test for each of the plots by adding a quadratic term and testing the quadratic to be zero (Tukey's test for non-additivity)
and overlays a smooth curve for the residuals.
...
Both quadratic and smoother-based curves were fitted to the residual plots for `glmFitOD3` (recall that this model was `Count ~ XPos*Phase + YPos*Phase + Depth + DistCoast`). In this case, there appear to be some nonlinearities on the link scale as curvature is apparent in all model covariates and the test statistic for the coefficients associated with the quadratic term ($x^2_j$) for each covariate are all extremely large and the associated $p$-values are small and significant.
...
So far, we've assumed that the model errors are independent (i.e. that our observations are independent). If the model does not capture the patterns in the response data, then some of this pattern will remain in the model residuals and present itself as positive/negative auto-correlation.

For instance, we've already seen in previous exploratory work (Figure \@ref(fig:nysted-random-transects)) that animals along transects are often clustered together and counts observed close together in time are more similar than counts distant in time/space.
...
ggplot() +
    geom_point(aes(x=XPos, y=YPos, colour=TransectID), 
               data=df[1:200, ]) +
    geom_point(aes(x=XPos, y=YPos, size=log(Count/Area)), 
               colour='black', 
               data=subset(df[1:200, ], Count>0))
```

To some extent, model covariates will be able to explain the similarity in these counts along transects (Figure \@ref(fig:observed-fitted-200)).
However, much of this pattern tends to remain unexplained by the model and is found in the model residuals (Figure \@ref(fig:residuals-200)). 
...
```{r observed-fitted-200, fig.cap="Observed bird counts (blue line) and model (`glmFitOD3`) fitted values (green line) for the first 200 observations."}
...
```{r residuals-200, fig.cap="Pearson residuals for the first 200 observations for model `glmFitOD3`."}
ggplot(mdlSummary[1:200, ]) +
...
We can visualise this problem by an autocorrelation function plot, which 
depicts the estimated correlation between the residuals and the
lag $k$ residuals (see [acf](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/acf) in R). 

```{r acf-glmFitOD3, fig.cap="Autocorrelation plot for the residuals of `glmFitOD3`.", fig.width=5, fig.height=5}
acf(residuals(glmFitOD3, type="pearson"), main="glmFitOD3")
```
For lags up to about $k=4$ the residuals remain correlated. For uncorrelated residuals we expect acf=1 
at lag=0, and acf=0 otherwise.

### Diagnosing autocorrelation using the runs test

Non-independence in model residuals can be diagnosed using the Wald-Wolfowitz runs test [@WaldWolfowitz1943]:
...
```{r sign-glmFitOD3, fig.cap="The sign of model (`glmFitOD3`) residuals for the first 200 observations."}
ggplot(mdlSummary[1:200, ]) +
...
* Model B: The linear predictor is a function of a measured covariate $x$ and a sine wave/function 
to mimic factors that exist in the environment that affect bird numbers but that we cannot directly observe.
The $\beta$'s are assumed the same in both models.
...
Positive correlation in model residuals can lead us to conclude that irrelevant variables are important. To illustrate this, let's now
add a covariate which is unrelated to the response and entirely made of randomly generated noise ($x_{\text{noise}}$). 
...
We can now see that $x_{\text{noise}}$ was found to be significantly related to $y^{(B)}$ but not to $y^{(A)}$. This is 
problematic, as $x_{\text{noise}}$ offers no predictive capacity whatsoever, yet it would've been retained in model B.
...
* **Model A** 
    + $x_{\text{noise}}$ was falsely identified as statistically significant just `r round((keepNoiseA/NRepeat)*100, 2)`% of the time 
    (in line with the expected false positive rate of 5%).
...
Let's see if positive residual correlation is also apparent in our Nysted model `glmFitOD3`:

```{r}
runs.test(residuals(glmFitOD3, type="pearson"))
```

We can update this model to incorporate the autocorrelation in model residuals using Generalised Estimating Equations (GEEs)  which will be introduced later.

## Mean-variance relationship

We can assess the mean-variance relationship by looking at the fitted values versus 
the residuals. To ease visual inspection we "bin" the fitted data and compute the 
variance of the residuals for each "bin".
...
We can look at the "raw" residuals ($\hat{\epsilon_i}^{\text{raw}}=y_i-\hat{y_i}$),
in which case, if the model assumptions are met, 
we should observe the assumed mean-variance relationship. So for the Poisson model 
we should see the variance increasing with the mean (Poisson model assumes that the variance is equal to the mean).
...
plot(fitted(mdl), residuals(mdl, type="response"), 
     pch=19, cex=0.5, ylab="Raw residuals")
...
resvar <- tapply(residuals(mdl, type="response"), xbin, var)
# Plot
plot(xmean, resvar, xlab="Mean fitted values", 
     ylab="Variance of raw residuals", pch=19)
abline(a=0, b=1, col='grey', lty=2, lwd=3)
```

For real-world data, inspecting the "raw" residuals can be difficult. Instead, we can use the Pearson residuals,
which are standardised residuals, so they have approximately constant variance (for Poisson model $\hat{\epsilon_i}^{\text{pearson}}=\frac{y_i-\hat{y_i}}{\sqrt{\hat{y_i}}}$). 
Thus, in this case if the model assumptions are met, we should 
observe a constant relationship between the mean and the variance (same as for ordinary linear regression models).

```{r mean-var-pearson-fake-data, fig.cap="Mean-variance relationship for a Poisson simulated dataset using the Pearson residuals.", fig.width=8, fig.height=4}
par(mfrow=c(1, 2))

# Plot 1 - Fitted values vs Pearson residuals
plot(fitted(mdl), residuals(mdl, type="pearson"), 
     pch=19, cex=0.5, ylab="Pearson residuals")

# Plot 2 - Mean binned fitted values vs var of Pearson residuals
# "Bin" the x-axis
xbin <- cut(x=fitted(mdl), 
            breaks=quantile(fitted(mdl), 
                            probs=c(seq(0, 1, length=20))))
...
xmean <- tapply(fitted(mdl), xbin, mean) 
# Compute var of residuals within xbin
resvar <- tapply(residuals(mdl, type="pearson"), xbin, var)
# Plot
plot(xmean, resvar, xlab="Mean fitted values", 
     ylab="Variance of Pearson residuals", pch=19)
abline(h=mean(resvar), col='grey', lty=2, lwd=3)
```

Let's inspect the mean-variance relationship for the `glmFitOD3`

```{r mean-var-glmFitOD3, fig.cap="Mean-variance relationship for the `glmFitOD3` model using the Pearson residuals.", fig.width=8, fig.height=4}
par(mfrow=c(1, 2))
...
There still seem to be a strong mean-variance relationship when looking at the Pearson residuals.
This suggests that at least some of our modelling assumptions aren't met, and that we may 
have to adjust our model specification to account for a different mean-variance relationship.
...
- the red line is the mean of these correlations across all transects and is simlilar to what you would see from running the usual `acf()` command.

```{r}
runACF(block = wfdata$transect.id, model = simpleModel,
       suppress.printout=TRUE)
```
In this case there is evidence that some transects show very high correlation 
...
Here we also do a runs test to assess for correlation in the model residuals.  Since our data are over-dispersed, we should use the empirical distribution for assessment:
...
empdist<-getEmpDistribution(500, simData, simpleModel, data=wfdata, dots=FALSE, plot = TRUE)
```
...
```{r eval=TRUE}
timeInfluenceCheck(finalmod, id = wfdata$blockid)
```

```{r, eval=TRUE, fig.width=9, fig.height=6}
inflpoints<-runInfluence(finalmod, id = wfdata$blockid)
```
