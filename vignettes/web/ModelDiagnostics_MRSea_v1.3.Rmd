...
To some extent, model covariates will be able to explain the similarity in these counts along transects. However, much of this pattern tends to remain unexplained by the model and is found in the model residuals (Figure \@ref(fig:residuals-200)).
...
There still seem to be a strong mean-variance relationship when looking at the Pearson residuals. This suggests that at least some of our modelling assumptions aren't met, and that we may have to adjust our model specification to account for a different mean-variance relationship. For example, we are underestimaiting the variance of the largest fitted values. 
...
# Diagnostics specifically for `gamMRSea` models
...
-   Non-independence in model residuals can be diagnosed using the Wald-Wolfowitz runs test :
-   This test assigns a "sign" to each residual: positive residuals are labelled +1's and negative residuals are labelled -1's.
-   The number of uninterrupted strings ("runs") of positive and negative residuals when looked at in sequence (e.g. time order) is then calculated.
-   Too few (long) runs provide evidence of positive correlation, while too many (short) runs provide evidence of negative correlation.
...
timeInfluenceCheck(model = simpleModel, 
                   id = nysted.analysisdata$transect.id)
```

```{r, eval=TRUE, fig.width=9, fig.height=6}
inflpoints<-runInfluence(simpleModel, id = wfdata$blockid)
```
