...
"get.measure.mn" <- function(fitnessMeasure,measures,out.lm, initDisp, cv.opts){
  ...
  if(fitnessMeasure=="AICc"){       
    # fitStat <- AICc(out.lm)
    stop('Fitness measure not supported for multinomial.  Please use AIC or BIC')
  }
  ...
  #cat("Evaluating new fit: ", fitStat, "\n")
  ### Not sure how this will work with moultinomial appears to be dealing with NAs
  if(is.na(fitStat)){
    fitStat<- tempMeasure + 1000
    cat("Change Fit due to NA: ", fitStat, "\n")
  }
