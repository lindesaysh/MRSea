...
"improve.step_2d" <- function(gap,knotDist,radii,dists,explData,num,response,knotgrid,maxIterations,fitnessMeasure, point,knotPoint,position,aR,BIC,track,out.lm,improveNudge,tol=0,baseModel,radiusIndices,models, interactionTerm, data, initDisp, fit.opts, basis, printout){
  if (isS4(baseModel)){
    ...
    for (k in 1:num) {
      ...
      for (j in nhbrs){
        if ((length(otherKnots)==0) || ( min(knotDist[j,otherKnots])>=gap)) {
          ...
          output<-fit.thinPlate_2d(fitnessMeasure, dists,tempR,radii,baseModel,radiusIndices,models, fitStat, interactionTerm, data, initDisp, fit.opts, basis)
