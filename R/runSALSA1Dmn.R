...
#' 
#' 
#initialModel=test_salsa_init_mn
#salsa1dlist=salsa1dlist1
#varlist=c("kt1")
#datain=data_mn
#removal=F
#suppress.printout=F
...
runSALSA1Dmn<-function(initialModel, salsa1dlist, varlist, factorlist=NULL, predictionData=NULL, varlist_cyclicSplines=NULL, splineParams=NULL, datain, removal=FALSE, panelid=NULL, suppress.printout=FALSE){
  ...
  winHalfWidth = 0
  ### this does not work for either multinom or vglm
  ## for vglm initialModel@family@vfamily[1] gives similar info
  if (isS4(initialModel)) {
    family <- initialModel@family@vfamily[1]
  ...
  # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ### not checked how this is affected by multinomial
  ...
  ### get dispersion edited to calculate for multinomial model
  ...
  for (i in 2:(length(varlist)+1)){
    ...
    if(removal==TRUE){
      ...
      if (isS4(baseModel)){
        cv_without<-cv.gamMRSea.mn(data, baseModel, K=salsa1dlist$cv.opts$K, cost=salsa1dlist$cv.opts$cost)$delta[2] 
      } else {
...
#      if(cvid==2){
  ...
  counter<-1
  if (isS4(baseModel)){
    origfamily<-initialModel@family@vfamily[1]
  } else {
    origfamily<-initialModel$family$family
  }

  if (isS4(baseModel)){
    outModel<-eval(parse(text=paste("update(baseModel, .~., family=",substitute(origfamily), ")",  sep='')))
  } else {
