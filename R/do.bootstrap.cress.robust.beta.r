...
do.bootstrap.cress.robust.beta<-function(model.obj, predictionGrid, splineParams=NULL, g2k=NULL, B, robust=T,name=NULL, seed=12345, nCores=1, cat.message=TRUE){
  ...
  if(nCores>1){
    ...
    Routputs<-parLapply(myCluster, 1:B, function(b){
      ...
      n.coeff<-length(coefficients(model.obj))-1
      est<- coefficients(model.obj)[1:n.coeff]
      if(robust==T){
        vbeta<-summary(model.obj)$vcov[1:n.coeff,1:n.coeff]
        samplecoeff<-NULL
        try(samplecoeff<- as.numeric(rmvnorm(1,est,vbeta, method='svd')))
        if(is.null(samplecoeff)){
          vbeta<-as.matrix(nearPD(as.matrix(summary(model.obj)$vcov))$mat)[1:n.coeff,1:n.coeff]
      ...
      m <- model.frame(Terms, predictionGrid, xlev = model.obj$xlevels)
      modmat <- model.matrix(Terms, m)
      rpreds <- model.obj$link$mean$linkinv(modmat %*% samplecoeff)
