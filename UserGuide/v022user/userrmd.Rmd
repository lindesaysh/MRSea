---
title: "Vignette for the ```MRSea``` Package (v0.2.2)"
...
Scott-Hayward, L.A.S., Oedekoven, C.S., Mackenzie, M.L. and Walker, C.G. (2016). Vignette for the MRSea Package v0.2.2: Statistical Modelling of bird and cetacean distributions in offshore renewables development areas. Centre for Research into Ecological and Environmental Modelling, University of St Andrews.
...
Use the built in summary function to look at the summary of the model.  This function differs from ```summary``` as it shortens the variable names for ease of viewing the output.

```{r }
summary.m(salsa1dOutput$bestModel,varlist = c("depth"), facls=c('season', 'impact'))
```
...
par(mfrow=c(2,2))
runPartialPlots(model = geeModel, data = data, factorlist.in = 
